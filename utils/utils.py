...
def save_model(args, cfg, model, filename, epoch, steps, remove_previous=True):
    if remove_previous:
    ...
    for k, v in ckpt.items():
        ...
        if k.endswith('kernel'):
            ...
            if "downsample" in k:
                ...
                # depthwise conv
                kv, dim = v.shape
                kernel_size = int(round(kv**(1/3), 0))
            
                new_v = v.permute(1, 0).reshape(dim, 1, kernel_size, kernel_size, kernel_size)    
